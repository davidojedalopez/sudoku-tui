...
func buildTheme(name string, p Palette, strings ThemeStrings) *Theme {
	...
	t.Cell = CellStyles{
		...
		Highlight:     lipgloss.NewStyle().Background(p.BorderHeavy),
		NoteHighlight: lipgloss.NewStyle().Background(p.Border).Foreground(p.FgDim),
		BandHighlight: lipgloss.NewStyle().Background(p.Border),
